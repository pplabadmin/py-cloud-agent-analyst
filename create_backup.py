...
if __name__ == "__main__":
    # Get the directory of the script to set as the default source.
    current_dir = os.path.abspath(os.path.dirname(__file__))

    # Check for command-line arguments to allow custom paths.
    if len(sys.argv) > MIN_ARGS_COUNT:
        root_source_path = sys.argv[1]
        root_dest_path = sys.argv[2]
    else:
        # Use the current directory as source and a 'backup' folder as
        # destination by default.
        root_source_path = current_dir
        root_dest_path = os.path.join(current_dir, "backup")

    # Start the copy process with the specified paths.
