...
def ensure_utf8(file_path: str):
    ...
    try:
        ...
        if results is None:
            msg = f"Inconsistencia crítica: No se pudo determinar el origen de {file_path}"
        ...
        try:
            ...
            msg = f"Fallo en consistencia: No se pudo convertir {file_path} ({e})"
...
def validate_and_process(
    ...
    if not files:
        msg = f"No se encontraron archivos en {folder_path}"
    ...
    for file_path in files:
        ...
        if missing_cols:
            msg = f"CRÍTICO: Al archivo {os.path.basename(file_path)} le faltan columnas: {missing_cols}"
